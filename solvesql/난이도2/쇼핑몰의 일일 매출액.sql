...
-- 정답
select 
  date_format(a.order_purchase_timestamp,'%Y-%m-%d') as dt,
  round(sum(b.payment_value),2) as revenue_daily
from olist_orders_dataset a
join olist_order_payments_dataset b
on a.order_id = b.order_id
where date_format(a.order_purchase_timestamp,'%Y-%m-%d') >= '2018-01-01'
group by dt
order by dt
...
/*
회고
- group by와 order by에 select의 칼럼을 등록 가능
- MySQL의 경우 날짜 포맷 설정에선 date_format을 사용
*/